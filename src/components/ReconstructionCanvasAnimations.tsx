...
export function FeatureMatchingCanvas() {
  ...
  const draw = (ctx: CanvasRenderingContext2D, w: number, h: number, t: number) => {
    ...
    if (m === "detect") {
      ...
      ctx.fillStyle = C.primary; ctx.font = "9px monospace"; ctx.textAlign = "center";
      ...
      ctx.fillStyle = C.mutedFg; ctx.font = "9px monospace"; ctx.textAlign = "left";
      ctx.fillText(`SIFT keypoints: ${visible}/${NUM_KP}`, 20, h - 6);
    } else if (m === "scale") {
      ctx.fillStyle = C.primary; ctx.font = "10px monospace"; ctx.textAlign = "center";
      ...
      for (let oct = 0; oct < octaves; oct++) {
        ...
        for (let lv = 0; lv < levels; lv++) {
          ...
          ctx.strokeStyle = C.primaryA(0.4); ctx.lineWidth = 1;
          ctx.strokeRect(x, y, cw, ch);
          ctx.fillStyle = C.primary; ctx.font = "7px monospace"; ctx.textAlign = "center";
          ctx.fillText(`σ=${sigma.toFixed(1)}`, x + cw / 2, y + ch + 10);

          if (lv < levels - 1) {
            const pulse = (Math.sin(t * 3 + lv + oct * 4) + 1) / 2;
            ctx.fillStyle = C.accentA(0.3 + pulse * 0.5);
            ctx.fillRect(x + cw + 1, y, 3, ch);
          }
        }
      }
      ctx.fillStyle = C.accentA(0.8); ctx.font = "8px monospace"; ctx.textAlign = "right";
      ...
      for (let i = 0; i < Math.min(visMatches, NUM_KP); i++) {
        ...
        ctx.strokeStyle = isGood ? C.green : C.redA(0.5);
        ...
        ctx.fillStyle = isGood ? C.green : C.redA(0.5); ctx.fill();
        ...
        if (isNew) {
          ...
          ctx.fillStyle = parseFloat(ratio) < 0.75 ? C.green : C.red;
      ...
      ctx.fillStyle = C.green; ctx.font = "8px monospace"; ctx.textAlign = "left";
      ...
      for (let i = 0; i < NUM_KP; i++) {
        ...
        ctx.strokeStyle = isInlier ? C.green : C.redA(0.3);
        ctx.lineWidth = isInlier ? 1.5 : 0.7;
        if (!isInlier) ctx.setLineDash([3, 3]);
        ctx.beginPath(); ctx.moveTo(lx, ly); ctx.lineTo(rx, ry); ctx.stroke();
        ctx.setLineDash([]);
        ctx.beginPath(); ctx.arc(lx, ly, 3, 0, Math.PI * 2);
        ctx.fillStyle = isInlier ? C.green : C.redA(0.5); ctx.fill();
        ctx.beginPath(); ctx.arc(rx, ry, 3, 0, Math.PI * 2); ctx.fill();
        ctx.restore();
      }
      ...
      ctx.fillStyle = C.mutedFg; ctx.font = "8px monospace";
...
export function EpipolarGeometryCanvas() {
  ...
  const draw = (ctx: CanvasRenderingContext2D, w: number, h: number, t: number) => {
    ...
    if (m === "basic") {
      ...
      [{ cx: lCamX, col: C.primary, colA: C.primaryA, lbl: "Left" },
       { cx: rCamX, col: C.accent, colA: C.accentA, lbl: "Right" }].forEach(({ cx, col, colA, lbl }) => {
        ctx.save();
        ctx.shadowColor = col; ctx.shadowBlur = 10;
        ctx.fillStyle = colA(0.15); ctx.strokeStyle = col; ctx.lineWidth = 1.5;
      ...
      ctx.shadowBlur = 0;
      ctx.fillStyle = C.accent; ctx.font = "8px monospace"; ctx.textAlign = "right";
      ...
      ctx.shadowBlur = 0;
      ctx.fillStyle = C.fg; ctx.font = "bold 9px monospace"; ctx.textAlign = "center";
      ...
      ctx.strokeStyle = C.border; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = C.primary; ctx.font = "12px monospace"; ctx.textAlign = "center";
      ...
      ctx.strokeStyle = C.border; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = C.primary; ctx.font = "11px monospace"; ctx.textAlign = "center";
      ctx.fillText("E = U · diag(1,1,0) · Vᵀ", cx, 38);
      ctx.fillStyle = C.mutedFg; ctx.font = "9px monospace";
      ...
      solutions.forEach((sol, i) => {
        ...
        ctx.save();
        ctx.shadowColor = sol.valid ? C.green : C.red;
        ctx.shadowBlur = sol.valid ? 12 + pulse * 8 : 3;
        ctx.fillStyle = sol.valid ? C.greenA(0.1) : C.redA(0.06);
        ctx.strokeStyle = sol.valid ? C.green : C.redA(0.4);
        ctx.lineWidth = sol.valid ? 2 : 1;
        ctx.beginPath(); (ctx as any).roundRect(px - 55, py - 30, 110, 60, 5); ctx.fill(); ctx.stroke();
        ctx.shadowBlur = 0;
        ctx.fillStyle = sol.valid ? C.green : C.redA(0.6);
        ctx.font = "bold 10px monospace"; ctx.textAlign = "center";
        ctx.fillText(sol.label, px, py - 8);
        ctx.fillStyle = sol.valid ? C.fg : C.mutedFg;
        ctx.font = "9px monospace";
        ctx.fillText(sol.valid ? "✓ positive depth" : "✗ behind camera", px, py + 8);
        if (sol.valid) {
          ctx.fillStyle = C.green; ctx.font = "bold 8px monospace";
          ctx.fillText("← SELECTED", px, py + 22);
        }
        ctx.restore();
      });

      ctx.fillStyle = C.mutedFg; ctx.font = "8px monospace"; ctx.textAlign = "center";
      ...
      for (let i = 0; i < 8; i++) {
        ...
        ctx.strokeStyle = active ? C.accent : C.accentA(0.4);
      ...
      ctx.strokeStyle = C.border; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = C.green; ctx.font = "9px monospace"; ctx.textAlign = "center";
      ...
      for (let i = 0; i < 8; i++) {
        ...
        ctx.strokeStyle = active ? C.green : C.greenA(0.4);
        ctx.lineWidth = active ? 2 : 0.8;
      ...
      ctx.strokeStyle = C.border; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = C.primary; ctx.font = "bold 10px monospace"; ctx.textAlign = "center";
      ctx.fillText("H, H′", w / 2, arrowY - 1);
      ctx.fillStyle = C.mutedFg; ctx.font = "7px monospace";
...
const sfmPoints3D = Array.from({ length: 200 }, (_, i) => ({
  ...
  color: `hsla(${180 + i * 2}, 70%, ${40 + rnd(-10, 20)}%, 1)`,
  size: rnd(1.5, 3.5),
}));
const NUM_CAMS = 8;
  ...
  const draw = (ctx: CanvasRenderingContext2D, w: number, h: number, t: number) => {
    ...
    if (m === "pipeline" || m === "sparse") {
      ...
      for (let x = -4; x <= 4; x++) {
        ...
        ctx.strokeStyle = C.primary; ctx.lineWidth = 0.4;
      ...
      sfmCameras.slice(0, Math.min(visibleCams, NUM_CAMS)).forEach((cam, i) => {
        ...
        const col = isNew ? C.accent : C.primary;
        const colA = isNew ? C.accentA : C.primaryA;
        ctx.save();
        ctx.shadowColor = col; ctx.shadowBlur = isNew ? 14 : 5;
        ctx.fillStyle = colA(0.15);
      ...
      ctx.fillStyle = C.bg(0.85);
      ctx.beginPath(); (ctx as any).roundRect(12, 12, 200, 48, 4); ctx.fill();
      ctx.fillStyle = C.primary; ctx.font = "9px monospace"; ctx.textAlign = "left";
      ...
      cams.forEach((cam, ci) => {
        ...
        ctx.save();
        ctx.shadowColor = C.primary; ctx.shadowBlur = 6;
        ctx.fillStyle = C.primaryA(0.15); ctx.strokeStyle = C.primary; ctx.lineWidth = 1.5;
      ...
      ctx.fillStyle = C.fg; ctx.font = "9px monospace"; ctx.textAlign = "right";
      ctx.fillText(`RMS error: ${totalErr}px`, w - 22, 50);

      ctx.fillStyle = C.mutedFg; ctx.font = "8px monospace"; ctx.textAlign = "left";
      ...
      [{ c: cam1, col: C.primary, colA: C.primaryA }, { c: cam2, col: C.accent, colA: C.accentA }].forEach(({ c, col, colA }) => {
      ...
      [{ c: cam1, col: C.primary, colA: C.primaryA, lbl: "C₁" }, { c: cam2, col: C.accent, colA: C.accentA, lbl: "C₂" }].forEach(({ c, col, colA, lbl }) => {
        ctx.save(); ctx.shadowColor = col; ctx.shadowBlur = 8;
        ctx.fillStyle = colA(0.15); ctx.strokeStyle = col; ctx.lineWidth = 2;
      ...
      ctx.strokeStyle = C.border; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = C.primary; ctx.font = "10px monospace"; ctx.textAlign = "left";
...
function depthColor(d: number, a = 1) {
  ...
  const s = lerp(85, 90, Math.abs(t - 0.5) * 2);
  const l = lerp(55, 50, t);
  return `hsla(${h}, ${s}%, ${l}%, ${a})`;
}
...
export function MVSCanvas() {
  ...
  const draw = (ctx: CanvasRenderingContext2D, w: number, h: number, t: number) => {
    ...
    if (m === "sweep") {
      ...
      ctx.fillStyle = C.mutedFg; ctx.font = "9px monospace"; ctx.textAlign = "center";
      ...
      for (let d = 0; d < DMAX; d++) {
        ...
        ctx.fillStyle = isCurrent ? C.primary : C.primaryA(0.25);
        if (isCurrent) { ctx.shadowColor = C.primary; ctx.shadowBlur = 6; }
      ...
      if (iteration === 1) {
        ...
        ctx.save(); ctx.shadowColor = C.accent; ctx.shadowBlur = 8;
        ctx.beginPath(); ctx.arc(px, py, 3, 0, Math.PI * 2);
        ctx.fillStyle = C.accent; ctx.fill(); ctx.shadowBlur = 0; ctx.restore();
      }

      const mse = (noiseScale * 0.15).toFixed(4);
      ctx.fillStyle = C.bg();
      ctx.beginPath(); (ctx as any).roundRect(20, 26 + sceneH * cellH + 8, w - 40, 40, 4); ctx.fill();
      ctx.fillStyle = C.green; ctx.font = "9px monospace"; ctx.textAlign = "left";
      ...
      ctx.fillStyle = C.mutedFg; ctx.font = "7px monospace"; ctx.textAlign = "left";
      ...
      ctx.fillStyle = C.accent; ctx.font = "9px monospace"; ctx.textAlign = "left";
...
export function NeRFCanvas() {
  ...
  const draw = (ctx: CanvasRenderingContext2D, w: number, h: number, t: number) => {
    ...
    if (m === "ray") {
      ...
      ctx.save();
      ctx.shadowColor = C.primary; ctx.shadowBlur = 10;
      ctx.fillStyle = C.primaryA(0.12); ctx.strokeStyle = C.primary; ctx.lineWidth = 2;
      ctx.beginPath(); (ctx as any).roundRect(camX - 30, camY - 18, 40, 36, 4); ctx.fill(); ctx.stroke();
      ctx.shadowBlur = 0;
      ctx.fillStyle = C.primary; ctx.font = "8px monospace"; ctx.textAlign = "center";
      ctx.fillText("Camera", camX - 10, camY + 3); ctx.restore();

      const planeX = camX + 24;
      ctx.fillStyle = C.primaryA(0.06);
      ctx.fillRect(planeX, camY - 50, 3, 100);
      ctx.strokeStyle = C.primaryA(0.3); ctx.lineWidth = 1;
      ...
      for (let ri = 0; ri < NUM_RAYS; ri++) {
        ...
        ctx.strokeStyle = isActive ? C.primaryA(0.6) : C.primaryA(0.15);
        ...
        if (isActive) {
          ...
          ctx.fillStyle = C.accent; ctx.shadowColor = C.accent;
      ...
      ctx.save(); ctx.shadowColor = C.accent; ctx.shadowBlur = 10;
      ...
      ctx.fillStyle = C.accent; ctx.font = "8px monospace";
      ...
      const layers = [
        { n: 6, label: "γ(x) 60-D", color: C.primary, colorA: C.primaryA, x: 0.08 },
        { n: 6, label: "FC 256", color: C.mutedFg, colorA: (a: number) => `hsla(215, 20%, 60%, ${a})`, x: 0.2 },
        { n: 6, label: "FC 256", color: C.mutedFg, colorA: (a: number) => `hsla(215, 20%, 60%, ${a})`, x: 0.32 },
        { n: 6, label: "FC 256", color: C.mutedFg, colorA: (a: number) => `hsla(215, 20%, 60%, ${a})`, x: 0.44 },
        { n: 6, label: "FC+skip", color: C.purple, colorA: C.purpleA, x: 0.56 },
        { n: 4, label: "σ+feat", color: C.accent, colorA: C.accentA, x: 0.68 },
        { n: 4, label: "γ(d) 24-D", color: C.green, colorA: C.greenA, x: 0.8 },
        { n: 3, label: "RGB", color: C.red, colorA: C.redA, x: 0.92 },
      ];
      ...
      layers.forEach((layer, li) => {
        ...
        for (let ni = 0; ni < layer.n; ni++) {
          ...
          ctx.fillStyle = isActive ? layer.color : layer.colorA(0.4);
          ctx.fill(); ctx.shadowBlur = 0; ctx.restore();
          layerNeurons.push({ lx, ny });
        }
        neurons.push(layerNeurons);

        ctx.fillStyle = isActive ? layer.color : C.mutedFg;
      ...
      ctx.save();
      ctx.strokeStyle = C.purpleA(0.5); ctx.lineWidth = 1.5; ctx.setLineDash([4, 3]);
      ctx.beginPath();
      ctx.moveTo(layers[0].x * w, h * 0.15);
      ctx.quadraticCurveTo(w * 0.32, h * 0.05, layers[4].x * w, h * 0.15);
      ctx.stroke(); ctx.setLineDash([]);
      ctx.fillStyle = C.purple; ctx.font = "7px monospace"; ctx.textAlign = "center";
      ...
      const stages = [
        { x: 0.1, label: "Posed\nImages", color: C.primary, colorA: C.primaryA },
        { x: 0.28, label: "Sample\nRay", color: C.mutedFg, colorA: (a: number) => `hsla(215, 20%, 60%, ${a})` },
        { x: 0.46, label: "MLP\nQuery", color: C.purple, colorA: C.purpleA },
        { x: 0.64, label: "Volume\nRender", color: C.accent, colorA: C.accentA },
        { x: 0.82, label: "MSE\nLoss", color: C.red, colorA: C.redA },
        { x: 0.95, label: "Back\nprop", color: C.green, colorA: C.greenA },
      ];
      ...
      stages.forEach((s, i) => {
        ...
        ctx.fillStyle = s.colorA(isActive ? 0.15 : 0.08);
        ...
        if (i < stages.length - 1) {
          ctx.strokeStyle = s.colorA(0.25); ctx.lineWidth = 1;
      ...
      ctx.strokeStyle = C.red; ctx.lineWidth = 2; ctx.stroke(); ctx.restore();
      ...
      ctx.strokeStyle = C.green; ctx.lineWidth = 1.5; ctx.stroke(); ctx.restore();

      ctx.fillStyle = C.accent; ctx.font = "9px monospace"; ctx.textAlign = "left";
      ctx.fillText(`Epoch: ${epoch * 1000}  Loss: ${lossVal.toFixed(4)}  PSNR: ${psnr.toFixed(1)}dB`, gx + 6, gy + 14);
      ctx.fillStyle = C.red; ctx.fillText("— MSE", gx + 6, gy + 26);
      ctx.fillStyle = C.green; ctx.fillText("— PSNR", gx + 60, gy + 26);
    }
  };
  ...
  const draw = (ctx: CanvasRenderingContext2D, w: number, h: number, t: number) => {
    ...
    if (m === "gaussians") {
      ...
      sortedG.forEach((g, i) => {
        ...
        ctx.fillStyle = isActive ? C.mutedFg : C.border;
      ...
      ctx.shadowBlur = 0;
      ctx.fillStyle = C.fg; ctx.font = "7px monospace"; ctx.textAlign = "center";
      ...
      const ops = [
        { x: w * 0.22, y: h * 0.35, label: "CLONE", color: C.green, colorA: C.greenA, desc: "‖∇μ‖ > τ AND small\n→ duplicate + offset" },
        { x: w * 0.55, y: h * 0.35, label: "SPLIT", color: C.accent, colorA: C.accentA, desc: "‖∇μ‖ > τ AND large\n→ replace with 2 smaller" },
        { x: w * 0.85, y: h * 0.35, label: "PRUNE", color: C.red, colorA: C.redA, desc: "α < ε_α\n→ remove" },
      ];
      ...
      ops.forEach((op, i) => {
        ...
        ctx.fillStyle = op.colorA(isActive ? 0.15 : 0.08);
      ...
      ctx.strokeStyle = C.border; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = C.primary; ctx.font = "8px monospace"; ctx.textAlign = "left";
      ctx.fillText("Every 100 iters: check ‖∇μ‖ per Gaussian", 22, h - 42);
      ctx.fillStyle = C.green; ctx.fillText("  clone if small + large gradient", 22, h - 28);
      ctx.fillStyle = C.accent; ctx.fillText("  split if large + large gradient", 22, h - 16);
    } else if (m === "compare") {
      const splitX = w / 2;

      ctx.fillStyle = C.purpleA(0.05); ctx.fillRect(0, 0, splitX, h);
      ctx.fillStyle = C.primaryA(0.03); ctx.fillRect(splitX, 0, splitX, h);

      ctx.fillStyle = C.purple; ctx.font = "bold 10px monospace"; ctx.textAlign = "center";
      ...
      for (let l = 0; l < 5; l++) {
        for (let n = 0; n < 4; n++) {
          ...
          ctx.fillStyle = C.purpleA(0.3 + pulse * 0.5); ctx.fill();
        }
      }
      ...
      metrics.forEach((mt, i) => {
        const y = h * 0.58 + i * 20;
        ctx.fillStyle = i % 2 === 0 ? C.muted(0.2) : C.muted(0.1);
        ctx.fillRect(8, y, splitX - 16, 18);
        ctx.fillRect(splitX + 8, y, splitX - 16, 18);

        ctx.fillStyle = C.mutedFg; ctx.font = "8px monospace"; ctx.textAlign = "left";
        ctx.fillText(mt[0], 14, y + 13);
        ctx.fillStyle = C.purple; ctx.fillText(mt[1], splitX * 0.55, y + 13);
        ctx.fillStyle = C.mutedFg; ctx.fillText(mt[0], splitX + 14, y + 13);
        ctx.fillStyle = C.primary; ctx.fillText(mt[2], splitX + splitX * 0.55, y + 13);
      });
